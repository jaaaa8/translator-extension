...
function createContentVm(port, href = "https://x/", { clock = performance, imageCount = 1 } = {}) {
  ...
  return { context, port, image, images, runtimeMessages, liveOverlays: () => rendered.filter((element) => !element.removed), liveBubbles: () => rendered.filter((element) => !element.removed).flatMap((element) => element.children), moveOffscreen: () => { rect = { left: 900, top: 0, right: 1400, bottom: 800, width: 500, height: 800 }; }, moveOnscreen: () => { rect = { left: 0, top: 0, right: 500, bottom: 800, width: 500, height: 800 }; }, intersectionObserverCount: () => intersectionObservers };
}
...
(async () => {
  ...
  tick = 10; perJob.port.emit({ ...event(request, "A first"), job_id: request.jobs[0].job_id });
  tick = 25; perJob.port.emit({ ...event(request, "B first"), job_id: request.jobs[1].job_id });
  tick = 30; perJob.port.emit({ ...event(request, "B second"), job_id: request.jobs[1].job_id, block_id: "b2" });
  const renderMetrics = perJob.port.sent.filter((message) => message.type === "render_metric");
  assert.deepEqual(renderMetrics.map((row) => row.job_id), [request.jobs[0].job_id, request.jobs[1].job_id]);
  assert.equal(renderMetrics.filter((row) => row.job_id === request.jobs[0].job_id).length, 1);
  assert.ok(renderMetrics[0].first_overlay_ms < renderMetrics[1].first_overlay_ms);
  perJob.port.emit({ type: "scope_done", request_id: request.request_id, images: 2, translated: 2, failed: 0, page_metrics: request.jobs.map((job) => ({ job_id: job.job_id, first_overlay_ms: null })) });
  assert.deepEqual((await result).page_metrics.map((row) => row.first_overlay_ms), [10, 25]);
