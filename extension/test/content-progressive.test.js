...
function createContentVm(port) {
  ...
  return { context, port, lastTranslation: () => port.lastEmitted(), liveOverlays: () => rendered.filter((e) => !e.removed), liveBubbles: () => rendered.filter((e) => !e.removed).flatMap((e) => e.children), moveOffscreen: () => { rect = { left: 900, top: 0, right: 1400, bottom: 800, width: 500, height: 800 }; }, swapSource: (source) => { image.src = source; }, intersectionObserverCount: () => intersectionObservers };
}
