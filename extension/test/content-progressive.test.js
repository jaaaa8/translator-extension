...
function createContentVm(port, href = "https://x/", { clock = performance, imageCount = 1 } = {}) {
  let nextId = 0, rect = { left: 0, top: 0, right: 500, bottom: 800, width: 500, height: 800 }, intersectionObservers = 0, storageChanged;
  ...
  return { context, port, image, images, runtimeMessages, storageChanged: (changes) => storageChanged(changes), liveOverlays: () => rendered.filter((element) => !element.removed), liveBubbles: () => rendered.filter((element) => !element.removed).flatMap((element) => element.children), moveOffscreen: () => { rect = { left: 900, top: 0, right: 1400, bottom: 800, width: 500, height: 800 }; }, moveOnscreen: () => { rect = { left: 0, top: 0, right: 500, bottom: 800, width: 500, height: 800 }; }, intersectionObserverCount: () => intersectionObservers };
}
